...
export default function CompanyCard({ company, index }: CompanyCardProps) {
  ...
  return (
    <div
      ...
      className="group relative overflow-hidden rounded-2xl bg-gradient-card border border-white/5 p-6 cursor-pointer card-hover animate-slide-up"
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center shadow-lg`}>
          ...
          <div>
            ...
            <p className="text-xs text-surface-400">{company.headquartersLocation}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-surface-100">{company.totalApplications}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-surface-100">{company.totalOffers}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-sm font-bold text-white">{company.avgSuccessRate}%</span>
        </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-white/5">
        <div className="flex flex-wrap gap-1.5">
          {company.typicalRoles.slice(0, 2).map((role) => (
            <span key={role} className="text-xs px-2 py-0.5 bg-surface-800/80 rounded-md text-surface-300">
              {role}
            </span>
          ))}
        </div>
        <span className="text-xs font-medium text-primary-400 flex items-center gap-1 group-hover:gap-2 transition-all">
