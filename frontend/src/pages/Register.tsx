...
import { Sparkles, Mail, Lock, User, GraduationCap, BookOpen, ArrowRight, ArrowLeft, CheckCircle2, Shield, BarChart3 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen h-screen flex flex-col md:flex-row overflow-hidden">
      ...
      <div className="relative md:w-[55%] min-h-[200px] md:min-h-0 flex-shrink-0 flex items-center justify-center p-6 md:p-12 overflow-hidden"
        ...
        <div className="relative z-10 max-w-md w-full overflow-y-auto max-h-full py-4">
          {/* Logo */}
          <div className="flex items-center gap-3 mb-8 lg:mb-10">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center shadow-xl shadow-primary-500/20">
          ...
          <div className="space-y-3">
            {[
              ...
              { icon: CheckCircle2, text: 'Access 500+ verified interview experiences' },
            ].map(({ icon: Icon, text }) => (
                ...
                {s < 2 && (
                  <div className="flex-1 h-0.5 rounded-full overflow-hidden bg-surface-800/60">
                    <div className={`h-full rounded-full transition-all duration-500 ${
                      step > 1 ? 'w-full bg-gradient-to-r from-primary-500 to-accent-500' : 'w-0'
                    }`} />
                  </div>
                )}
              </div>
          ...
          <form onSubmit={step === 2 ? handleSubmit : (e) => { e.preventDefault(); handleNext(); }}>
            ...
            <div className="flex items-center gap-3 mt-6">
              ...
              <button type="submit" disabled={isLoading}
                className="flex-1 py-3 px-4 rounded-xl bg-gradient-to-r from-primary-600 to-primary-500 hover:from-primary-500 hover:to-primary-400 text-white font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-300 disabled:opacity-50 shadow-lg shadow-primary-600/20 hover:shadow-primary-500/30 hover:translate-y-[-1px] active:translate-y-0"
                id="register-submit">
                {isLoading ? (
